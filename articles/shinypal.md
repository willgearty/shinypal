...
shiny::observeEvent(input$add_filter, {
  ...
  # Use step_varname(ind) for the variable name so it matches the id shinypal
  # stores this step's dataset under.
  data <- shinymeta::metaReactive2(varname = step_varname(ind), {
...
[`add_shinypal_data_step()`](http://williamgearty.com/shinypal/reference/add_shinypal_data_step.md)
stores the result under `step_varname(ind)` (the `prefix` set in
[`shinypal_setup()`](http://williamgearty.com/shinypal/reference/shinypal_setup.md)
plus the step index; e.g., `data_1`), renders its generated code into
